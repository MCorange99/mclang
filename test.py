...
def test(folder: str):
    ...
    for entry in os.scandir(folder):
        ...
        if entry.is_file() and entry.path.endswith(porth_ext):
            ...
            sim_cmd = cmd_run_echoed([sys.executable, "./mclang.py", "sim", entry.path], capture_output=True)
            ...
            com_cmd = cmd_run_echoed([sys.executable, "./mclang.py", "com", "-r", "-s", entry.path], capture_output=True)
...
def record(folder: str, mode: str='sim'):
    for entry in os.scandir(folder):
        ...
        if entry.is_file() and entry.path.endswith(porth_ext):
            if mode == 'sim':
                output = cmd_run_echoed([sys.executable, "./mclang.py", "sim", entry.path], capture_output=True)
            elif mode == 'com':
                output = cmd_run_echoed([sys.executable, "./mclang.py", "com", "-r", "-s", entry.path], capture_output=True)
            else:
