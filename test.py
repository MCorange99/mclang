...
def test(folder):
    ...
    for entry in os.scandir(folder):
        ...
        if entry.is_file() and entry.path.endswith(ext):
            ...
            txt_path = entry.path[:-len(ext)] + ".txt"
            expected_output = None
            with open(txt_path, "rb") as f:
                expected_output = f.read()

            sim_output = cmd_run_echoed(["./mclang.py", "sim", entry.path], capture_output=True).stdout
            ...
            cmd_run_echoed(["./mclang.py", "com", entry.path])
            top_file = entry.path[:-len(ext)].split("/")[-1]
            path = entry.path[:-len(ext)].split("/")
            path.pop()
            path.append("build")
            path.append(top_file)
            path = "/".join(path)
            com_output = cmd_run_echoed([path], capture_output=True).stdout
